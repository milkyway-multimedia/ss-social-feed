...
class GooglePlus extends HTTP
{
    ...
    protected function handlePost(array $data, $settings = [])
    {
        $post = [
            ...
            'Title'        => isset($data['title']) ? $this->textParser()->text($data['title']) : '',
            'Type'         => isset($data['object']) && isset($data['object']['objectType']) ? $data['object']['objectType'] : '',
            'Content'      => isset($data['object']) && isset($data['object']['content']) ? '<p>' . $this->textParser()->text($data['object']['content']) . '</p>' : '',
            ...
            'Posted'       => isset($data['published']) ? \DBField::create_field('SS_Datetime',
                strtotime($data['published'])) : null,
        ];

        $post['Created'] = $post['Posted'];
        $post['StyleClasses'] = $post['Type'];

        $post['ReplyDescriptor'] = $post['ReplyCount'] == 1 ? _t('SocialFeed.REPLY', 'Reply') : _t('SocialFeed.REPLIES',
            'Replies');
        $post['LikesDescriptor'] = $post['LikesCount'] == 1 ? _t('SocialFeed.PLUS_ONE',
            'Plus One') : _t('SocialFeed.PLUS_ONES', 'Plus Ones');
        $post['ReshareDescriptor'] = $post['ReshareCount'] == 1 ? _t('SocialFeed.RESHARE',
            'Reshare') : _t('SocialFeed.RESHARES', 'Reshares');
